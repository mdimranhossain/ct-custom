...
if ( ! function_exists( 'ct_custom_woocommerce_product_columns_wrapper' ) ) {
	/**
...
if ( ! function_exists( 'ct_custom_woocommerce_product_columns_wrapper_close' ) ) {
	/**
...
if ( ! function_exists( 'ct_custom_woocommerce_wrapper_before' ) ) {
	/**
	 * Before Content.
	 *
...
if ( ! function_exists( 'ct_custom_woocommerce_wrapper_after' ) ) {
	/**
	 * After Content.
	 *
 ...
 *
	<?php
		if ( function_exists( 'ct_custom_woocommerce_header_cart' ) ) {
			ct_custom_woocommerce_header_cart();
		}
	?>
 */

if ( ! function_exists( 'ct_custom_woocommerce_cart_link_fragment' ) ) {
	/**
	 * Cart Fragments.
	 *
	 ...
	 */
	function ct_custom_woocommerce_cart_link_fragment( $fragments ) {
		ob_start();
		ct_custom_woocommerce_cart_link();
		$fragments['a.cart-contents'] = ob_get_clean();

		return $fragments;
	}
}
add_filter( 'woocommerce_add_to_cart_fragments', 'ct_custom_woocommerce_cart_link_fragment' );

if ( ! function_exists( 'ct_custom_woocommerce_cart_link' ) ) {
	/**
	 * Cart Link.
	 *
	 ...
	 * @return void
	 */
	function ct_custom_woocommerce_cart_link() {
		?>
		<a class="cart-contents" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'ct-custom' ); ?>">
			<?php
			$item_count_text = sprintf(
				/* translators: number of items in the mini cart. */
				_n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'ct-custom' ),
...
if ( ! function_exists( 'ct_custom_woocommerce_header_cart' ) ) {
	 ...
	 * @return void
	 */
	function ct_custom_woocommerce_header_cart() {
		if ( is_cart() ) {
		...
		<ul id="site-header-cart" class="site-header-cart">
			<li class="<?php echo esc_attr( $class ); ?>">
				<?php ct_custom_woocommerce_cart_link(); ?>
			</li>
			<li>
				<?php
				$instance = array(
